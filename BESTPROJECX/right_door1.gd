extends Area2D
onready var left_door2 = get_node("/root/World/left_door2")
...
func _on_right_door1_body_entered(body):
	if body.get_name() == "Player1" or body.get_name() == "Player2":
		body.set_global_position(left_door2.get_global_position()+Vector2(50,0))
		return
	var ball = body
	if ball.status==3:
		return
	elif ball.status==1:
		ball.set_global_position(left_door2.get_global_position())
