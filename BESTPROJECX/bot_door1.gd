extends Area2D
onready var top_door2 = get_node("/root/World/top_door2")
...
func _on_bot_door1_body_entered(body):
	if body.get_name() == "Player1" or body.get_name() == "Player2":
		body.set_global_position(top_door2.get_global_position()+Vector2(0,50))
		return

	var ball = body
	if ball.status==1:
		ball.set_global_position(top_door2.get_global_position())
