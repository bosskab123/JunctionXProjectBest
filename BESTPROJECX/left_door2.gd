...
func _on_left_door2_body_entered(body):
	if body.get_name() == "Player1" or body.get_name() == "Player2":
		body.set_global_position(right_door2.get_global_position()+Vector2(-50,0))
		return
	var ball = body
	if ball.status==3:
		return
	elif ball.status==1:
		ball.set_global_position(right_door2.get_global_position())
