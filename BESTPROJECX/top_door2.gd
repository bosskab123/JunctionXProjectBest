extends Area2D
onready var bot_door1 = get_node("/root/World/bot_door1")
...
func _on_top_door2_body_entered(body):
	if body.get_name() == "Player1" or body.get_name() == "Player2":
		body.set_global_position(bot_door1.get_global_position()-Vector2(0,50))
		return
	var ball = body
	if ball.status==1:
		ball.set_global_position(bot_door1.get_global_position())
